...
export const api = {
  ...
  async getAiReview(userId: number, date: string): Promise<AiReviewData> {
    ...
    if (points.length === 0) { points.push("Semua habit tercapai sempurna hari ini! Konsistensi 100% terjaga."); points.push("Pertahankan waktu tidur disiplin untuk memaksimalkan metabolisme tubuh."); }
    return { date, actionablePoints: points.slice(0, 5), rawSummary: points.slice(0, 5).join("\n") };
  },
};