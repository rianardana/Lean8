...
export const DailyCheckView: React.FC<DailyCheckViewProps> = ({ userId, onSaved }) => {
  ...
  useEffect(() => {
    let isMounted = true;
    setLoading(true);
    api.getDaily(userId, selectedDate).then((data) => {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const saved = await api.saveDaily(userId, log);
      setLog(saved);
      setSaveSuccess(true);
      if (onSaved) onSaved();
  ...
  const habits = [
    { id: "workout", title: "Workout / Movement", desc: "Latihan beban, kalistenik, atau jalan kaki 20+ menit", icon: Dumbbell, completed: log.workout, color: "text-emerald-400 bg-emerald-500/10 border-emerald-500/20" },
    { id: "ifCompleted", title: "Intermittent Fasting", desc: "Patuhi jendela fasting 16:8 atau sesuai target", icon: Clock, completed: log.ifCompleted, color: "text-teal-400 bg-teal-500/10 border-teal-500/20" },
    { id: "proteinCompleted", title: "Protein Target", desc: "Tercapai asupan protein harian untuk memelihara otot", icon: Utensils, completed: log.proteinCompleted, color: "text-cyan-400 bg-cyan-500/10 border-cyan-500/20" },
    { id: "waterCompleted", title: "Air Putih", desc: "Minum minimal 2.5 - 3 Liter air putih per hari", icon: Droplets, completed: log.waterCompleted, color: "text-blue-400 bg-blue-500/10 border-blue-500/20" },
    { id: "sleepCompleted", title: "Tidur Cukup", desc: "Tidur 7-8 jam berkualitas & matikan layar tepat waktu", icon: Moon, completed: log.sleepCompleted, color: "text-indigo-400 bg-indigo-500/10 border-indigo-500/20" },
    { id: "noSnack", title: "No Snack / Zero Junk", desc: "Bebas snack manis/olahan di luar jadwal makan utama", icon: Ban, completed: log.noSnack, color: "text-rose-400 bg-rose-500/10 border-rose-500/20" },
  ] as const;
      ...
      <div className="bg-slate-900/80 backdrop-blur-md border border-slate-800/80 rounded-3xl p-6 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 bg-slate-950 px-3.5 py-2 rounded-2xl border border-slate-800">
            ...
            <input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} className="bg-transparent text-xs font-mono text-slate-200 focus:outline-none cursor-pointer" />
          </div>
        </div>
          ...
          {habits.map((habit) => {
            ...
            return (
              <div key={habit.id} onClick={() => toggleHabit(habit.id as keyof Omit<DailyLogData, "id" | "date" | "notes" | "completedCount">)}
                className={`group cursor-pointer select-none rounded-2xl border p-4 transition-all duration-200 flex items-start justify-between gap-3 ${habit.completed ? "bg-slate-900/90 border-emerald-500/50 shadow-md shadow-emerald-500/5" : "bg-slate-900/40 border-slate-800/80 hover:border-slate-700/80"}`}>
                <div className="flex items-start gap-3">
                  <div className={`p-2.5 rounded-xl border ${habit.color}`}><Icon className="w-5 h-5" /></div>
                  <div className="space-y-1">
                    <h4 className={`text-sm font-semibold transition-colors ${habit.completed ? "text-slate-100" : "text-slate-300"}`}>{habit.title}</h4>
                    <p className="text-xs text-slate-500 leading-relaxed">{habit.desc}</p>
                  </div>
                </div>
                <div className={`w-6 h-6 rounded-full flex items-center justify-center border transition-all duration-200 shrink-0 ${habit.completed ? "bg-emerald-500 border-emerald-400 text-slate-950 scale-110 shadow-sm shadow-emerald-500/50" : "border-slate-700 bg-slate-950 text-transparent group-hover:border-slate-500"}`}>
