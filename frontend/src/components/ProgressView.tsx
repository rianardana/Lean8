      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            {successMsg && (
              <p className="text-xs text-emerald-400 flex items-center gap-1">
                <CheckCircle2 className="w-3.5 h-3.5" /> Berat berhasil tercatat!
              </p>
            )}
            ...
            <button
              ...
              className="w-full py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 text-slate-950 font-bold text-xs shadow-md shadow-emerald-500/20 hover:opacity-95 transition-all disabled:opacity-50 flex items-center justify-center gap-1.5"
            >
              <Plus className="w-4 h-4" />
              <span>{submitting ? "Menyimpan..." : "Simpan Log Berat"}</span>
            </button>
          </form>
        </div>
        ...
        <div className="lg:col-span-2 bg-slate-900/80 border border-slate-800/80 rounded-3xl p-6 space-y-4 flex flex-col justify-between">
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              ...
              <p className="text-lg font-black text-emerald-400">
                {weights.length ? `${weights[weights.length - 1].weight} kg` : "-"}
              </p>
            </div>
          </div>
          ...
          <div className="relative w-full h-44 bg-slate-950/60 rounded-2xl border border-slate-800/60 p-4 flex items-center justify-center">
            {loading ? (
              <span className="text-xs text-slate-500 animate-pulse">Memuat grafik...</span>
            ) : weights.length < 2 ? (
              <span className="text-xs text-slate-500">Minimal 2 entri berat untuk menampilkan trend grafik.</span>
            ) : (
              <svg viewBox="0 0 300 140" className="w-full h-full overflow-visible">
                ...
                {weights.map((w, index) => {
                  ...
                  return (
                    <g key={w.id || index}>
                      <circle cx={x} cy={y} r="4" fill="#090D16" stroke="#10B981" strokeWidth="2" />
                    </g>
                  );
                })}
              </svg>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-slate-300">
            <thead className="bg-slate-950 text-slate-400 font-mono uppercase text-[10px]">
              <tr>
                <th className="py-2.5 px-4 rounded-l-xl">Tanggal</th>
              ...
              {weights.map((w) => (
                <tr key={w.id || w.date} className="hover:bg-slate-800/30 transition-colors">
                  ...
                  <td className="py-3 px-4">
                    <span className="inline-block px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 text-[10px] font-mono border border-emerald-500/20">
                      Tercatat
                    </span>
                  </td>
                </tr>
